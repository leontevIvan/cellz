...
export class GameScene extends Phaser.Scene {
  ...
  private drawOcean(g: Phaser.GameObjects.Graphics, W: number, H: number) {
    ...
    g.fillStyle(0x082848, 0.30); g.fillRect(0, H * 0.58, W, H * 0.42);
    g.fillStyle(0x041828, 0.38); g.fillRect(0, H * 0.82, W, H * 0.18);

    // animated current bands
    const wt = this.time.now / 1800;
    for (let i = 0; i < 7; i++) {
    ...
    // caustic patches
    const ct = this.time.now / 2800;
    for (let row = 0; row < 3; row++) {
      for (let col = 0; col < 5; col++) {
        const i  = row * 5 + col;
        const cx = W * ((col + 0.5) / 5 + Math.sin(ct * 0.75 + i * 1.1) * 0.055);
        const cy = H * (0.03 + row * 0.20 + Math.cos(ct * 0.55 + i * 0.85) * 0.035);
        const cr = 28 + 14 * Math.abs(Math.sin(ct * 1.0 + i * 0.65));
        const al = 0.055 + 0.030 * Math.abs(Math.sin(ct * 0.85 + i));
        g.fillStyle(0x90d8f8, al);
        g.fillCircle(cx, cy, cr);
        g.fillStyle(0x90d8f8, al * 0.45);
        g.fillCircle(cx + cr * 0.55, cy + cr * 0.38, cr * 0.55);
        g.fillCircle(cx - cr * 0.48, cy + cr * 0.28, cr * 0.42);
      }
    }

    const st = this.time.now / 1200;
    for (let i = 0; i < 6; i++) {
      const sx = W * ((i + 0.5) / 6 + Math.sin(st + i * 0.8) * 0.04);
      g.fillStyle(0xb8e8ff, 0.055 + 0.03 * Math.abs(Math.sin(st * 1.2 + i)));
      g.fillTriangle(sx, 0, sx - 60, H * 0.28, sx + 60, H * 0.28);
    }
